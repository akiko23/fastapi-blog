...
@pytest.mark.asyncio
async def test_get_me(client: AsyncClient, user_service: UserService, config: BackendConfig):
    user = await user_service.create(UserCreate(email="user227@mail.ru", username="user226", password="12345"))
...
@pytest.mark.asyncio
async def test_patch_me(client: AsyncClient, user_service: UserService, config: BackendConfig):
    user = await user_service.create(UserCreate(email="user228@mail.ru", username="user227", password="12345"))
...
@pytest.mark.asyncio
async def test_get_user(client: AsyncClient, user_service: UserService, config: BackendConfig):
    common_user = await user_service.create(
        UserCreate(email="common_user@mail.ru", username="common_user226", password="12345")
    )
    super_user = await user_service.create(
        UserCreate(email="admin@mail.ru", username="admin226", password="123456", is_superuser=True)
    )
    ...
    cu_resp = await client.get(f"users/{super_user.id}", headers={
    ...
    su_resp = await client.get(f"users/{common_user.id}", headers={
